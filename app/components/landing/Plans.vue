  ...
  <section id="plans" class="border-b border-border/60 bg-background">
    <div class="mx-auto max-w-6xl px-6 py-24">
      ...
      <div class="grid gap-6 md:grid-cols-3">
        <div v-for="plan in plans" :key="plan.id" @click="emit('select', plan.id)" :class="[
          ...
          <img :src="planImages[plan.id]" :alt="`${plan.name} plan`" class="aspect-video w-full object-cover h-64" />
          ...
          <div class="px-6 py-8">
            ...
            <ul :class="[
              ...
              <li v-for="feature in plan.features" :key="feature" class="flex items-center gap-3 text-sm">
                <Check class="h-4 w-4 shrink-0" :class="plan.popular ? 'text-white' : 'text-foreground'" />
                {{ feature }}
              </li>
            </ul>
