...
// Pre-fill checkout form when plan is selected
watch(
  () => props.selectedPlan,
  (plan) => {
...
// Handle form submission and create checkout session
      ...
      <div class="mt-12 grid gap-8 lg:grid-cols-5">
        <aside class="lg:col-span-2 rounded-3xl border border-border bg-card p-6">
          <template v-if="plan">
            <div class="overflow-hidden rounded-2xl">
              <img :src="planImages[plan.id]" :alt="plan.name" class="aspect-4/3 w-full object-cover" />
            </div>
        ...
        <form @submit.prevent="handleSubmit" class="lg:col-span-3 rounded-3xl border border-border bg-card p-8">
          <div class="grid gap-5">
            <div>
              <label for="co-name" class="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                ชื่อ-นามสกุล
              </label>
              <input id="co-name" type="text" v-model="name" required placeholder="Jane Doe"
            ...
            <div>
              <label for="co-email" class="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                อีเมล
              </label>
              <input id="co-email" type="email" v-model="email" required placeholder="jane@example.com"
                class="mt-2 h-11 w-full rounded-full border border-border bg-background px-5 text-sm text-foreground outline-none transition-colors focus:border-primary" />
            </div>
            <div>
              <label for="co-addr" class="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                ที่อยู่จัดส่ง
              </label>
              <input id="co-addr" type="text" v-model="address" required placeholder="221B Baker Street, London"
                class="mt-2 h-11 w-full rounded-full border border-border bg-background px-5 text-sm text-foreground outline-none transition-colors focus:border-primary" />
            </div>
          </div>
