  ...
  <div class="min-h-screen bg-background font-sans">
    ...
    <main class="mx-auto max-w-6xl px-6 py-16">
      ...
      <div v-if="!plan" class="mt-16 rounded-sm border border-dashed border-border p-16 text-center">
        ...
        <h2 class="mt-6 text-xl font-light text-foreground">ไม่มีแผนสมาชิกที่ใช้งาน</h2>
        <p class="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">เลือกแผนเพื่อเริ่มรับผักโขมอบชีสสดใหม่ทุกวัน</p>
      ...
      <div v-else-if="activeMeta" class="mt-12 grid gap-6 lg:grid-cols-3">
        <section class="lg:col-span-2 rounded-sm border border-border bg-card p-8">
          <div class="flex items-start justify-between">
            <div>
              <p class="text-xs uppercase tracking-[0.25em] text-muted-foreground">แผนปัจจุบัน</p>
              <h2 class="mt-2 text-3xl font-light tracking-tight text-foreground">{{ activeMeta.name }}</h2>
            </div>
            <span class="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium uppercase tracking-wider text-primary">ใช้งานอยู่</span>
          </div>

          <div class="mt-10">
            <div class="flex items-baseline justify-between">
              <span class="text-5xl font-light tracking-tight text-foreground">{{ remaining }}</span>
              <span class="text-sm text-muted-foreground">จาก {{ activeMeta.days }} วัน</span>
            </div>
            ...
            <p class="mt-3 text-xs text-muted-foreground">ส่งแล้ว {{ used }} ที่ · เหลืออีก {{ remaining }} ที่</p>
          </div>

          <div class="mt-10 grid gap-6 sm:grid-cols-3 border-t border-border pt-8">
            <div>
              <p class="text-xs uppercase tracking-[0.2em] text-muted-foreground">เริ่ม</p>
            ...
            <div>
              <p class="text-xs uppercase tracking-[0.2em] text-muted-foreground">ต่ออายุ</p>
        ...
        <aside class="space-y-6">
          <div class="rounded-sm border border-border bg-card p-6">
            ...
            <p class="mt-3 text-lg font-medium text-foreground">
              พรุ่งนี้, {{ nextDelivery.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' }) }}
            </p>
            <p class="mt-1 text-xs text-muted-foreground">ออกจากเตา · สดได้ 24 ชม. ในตู้เย็น</p>
          </div>
          <div class="rounded-sm border border-border bg-card p-6">
            <div class="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-muted-foreground">
            ...
            <ul class="mt-4 space-y-3 text-sm text-foreground">
              <li class="flex justify-between"><span>จันทร์ – ศุกร์</span><span class="text-muted-foreground">8:00 AM</span></li>
              <li class="flex justify-between"><span>เสาร์</span><span class="text-muted-foreground">9:30 AM</span></li>
              <li class="flex justify-between"><span>อาทิตย์</span><span class="text-muted-foreground">วันหยุด</span></li>
            </ul>
          </div>
        </aside>
      </div>
